...
class_name Rock
## Asteroid that splits into smaller rocks when destroyed
...
# Track rock shape for variety
static var next_shape: int = 0
...
func _init(type: RockType = RockType.LARGE):
	...
	if abs(heading.x) < 0.1:
		heading.x = 0.1 if randf() > 0.5 else -0.1
	if abs(heading.y) < 0.1:
		heading.y = 0.1 if randf() > 0.5 else -0.1
...
func _on_area_entered(area: Area2D):
	"""Handle collision with bullets or ship"""
