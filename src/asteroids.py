...
class Asteroids():
    ...
    def __init__(self):
        ...
        self.ship = None
        self.lives = 0
        self.highscoreTab = [ ['ACE', 20000], ['SND', 10000], ['TRD', 5000] ]
        self.preInitials = 'AAA'
        self.scorePos = 0
        self.scoreChecked = False
    ...
    def checkScore(self):
        ...
        print(self.score)
        if self.score <= self.highscoreTab[2][1]:
            self.gameState = 'attract_mode'
            self.scoreChecked = True
        if self.score > self.highscoreTab[2][1] and self.score <= self.highscoreTab[1][1]:
            hlp[2][0] = self.preInitials
            hlp[2][1] = self.score
            self.scorePos = 2
            self.highscoreSet()
            self.highscoreTab = hlp
        if self.score > self.highscoreTab[1][1] and self.score <= self.highscoreTab[0][1]:
            ...
            self.highscoreSet()
            self.highscoreTab = hlp
        if self.score > self.highscoreTab[0][1]:
            hlp[2][0] = self.highscoreTab[1][0]
            hlp[2][1] = self.highscoreTab[1][1]
    ...
    def playing(self):
        if self.lives == 0:
            if self.scoreChecked == False:
                self.checkScore()
                #print(self.scoreChecked)
        else:
            self.processKeys()
            ...
            if event.type == QUIT:
                ...
                if self.gameState == 'playing':
                    ...
                    if event.key == K_RETURN:
                        self.initialiseGame()
                elif self.gameState == 'highscore_set':
